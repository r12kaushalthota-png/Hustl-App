/*
  # Simplify Task Status Flow

  1. Changes
    - Update status flow to: accepted → started → on_the_way → delivered → completed
    - Keep existing database columns compatible
    - Map statuses for backward compatibility

  2. Status Flow
    - Task doers: started → on_the_way → delivered
...
-- Update task_status_history constraint to allow simplified statuses
DO $$
BEGIN
  IF EXISTS (
